...
def _build_vader_calibration(db: Database) -> dict[str, float | int]:
    analyzer = SentimentIntensityAnalyzer()
    texts: list[str] = []
    texts.extend([f"{item.get('title', '')}. {item.get('message', '')}" for item in db["alerts"].find({}, {"title": 1, "message": 1})])
    texts.extend([str(item.get("description", "")) for item in db["replay_events"].find({}, {"description": 1})])
...
def _trend_samples(db: Database) -> tuple[np.ndarray, np.ndarray]:
    points = list(db["trend_points"].find({}, {"_id": 0}).sort([("coin_symbol", ASCENDING), ("ts", ASCENDING)]))
    by_symbol: dict[str, list[dict]] = defaultdict(list)
    for point in points:
        by_symbol[str(point["coin_symbol"])].append(point)

    features: list[list[float]] = []
    labels: list[int] = []

    for series in by_symbol.values():
        mention_values = [int(item["mentions"]) for item in series]
        ...
        for index in range(1, len(series)):
            ...
            current_mentions = int(current["mentions"])
            previous_mentions = int(previous["mentions"])
            current_price = float(current["price"])
            previous_price = float(previous["price"])
            mention_delta = current_mentions - previous_mentions
            price_return = (current_price - previous_price) / max(previous_price, 1e-9)
            is_spike = int(current_mentions > (series_mean + series_std))
            features.append([
                float(current_mentions),
                float(mention_delta),
...
def _train_trend_detector(db: Database) -> dict[str, float | int | str]:
...
def _price_direction_samples(db: Database) -> tuple[np.ndarray, np.ndarray]:
    points = list(db["trend_points"].find({}, {"_id": 0}).sort([("coin_symbol", ASCENDING), ("ts", ASCENDING)]))
    coins = {str(coin["symbol"]): coin for coin in db["coins"].find({}, {"_id": 0})}
    by_symbol: dict[str, list[dict]] = defaultdict(list)
    for point in points:
        by_symbol[str(point["coin_symbol"])].append(point)
    ...
    for symbol, series in by_symbol.items():
        ...
        hype = _safe_float(float(coin["hype_score"]) if coin else None)
        trust = _safe_float(float(coin["trust_score"]) if coin else None)
        for index in range(1, len(series) - 1):
            ...
            mention_delta = current_mentions - previous_mentions
            price_return = (current_price - previous_price) / max(previous_price, 1e-9)
            target_up = int(next_price > current_price)
            X.append(
                [
                    float(current_mentions),
                    float(mention_delta),
...
def _train_price_predictor(db: Database) -> dict[str, float | int | str]:
...
def _train_trust_regressor(db: Database) -> dict[str, float | int | str]:
    influencers = list(db["influencers"].find({}, {"_id": 0}))
    metrics = list(db["influence_metrics"].find({}, {"_id": 0}))

    metric_map: dict[str, dict[str, int]] = defaultdict(dict)
    for metric in metrics:
        metric_map[str(metric["influencer_handle"])][str(metric["metric"])] = int(metric["value"])

    X: list[list[float]] = []
    y: list[float] = []

    for influencer in influencers:
        influencer_metrics = metric_map.get(str(influencer["handle"]), {})
...
def _train_anomaly_detector(db: Database) -> dict[str, float | int | str]:
    points = list(db["trend_points"].find({}, {"_id": 0}).sort([("coin_symbol", ASCENDING), ("ts", ASCENDING)]))
    by_symbol: dict[str, list[dict]] = defaultdict(list)
    for point in points:
        by_symbol[str(point["coin_symbol"])].append(point)
    ...
    for series in by_symbol.values():
        for index in range(1, len(series)):
            previous = series[index - 1]
            current = series[index]
            current_mentions = float(current["mentions"])
            previous_mentions = float(previous["mentions"])
            current_price = float(current["price"])
            previous_price = float(previous["price"])
            mention_delta = (current_mentions - previous_mentions) / max(previous_mentions, 1)
            price_delta = (current_price - previous_price) / max(previous_price, 1e-9)
            rows.append([current_mentions, float(mention_delta), float(price_delta), float(current["sentiment"])])
...
def _train_influencer_graph(db: Database) -> dict[str, str | int | list[dict[str, float | str]]]:
    influencers = list(db["influencers"].find({}, {"_id": 0}))
    coins = list(db["coins"].find({}, {"_id": 0}))
    ...
    for influencer in influencers:
        ...
        max_links = max(1, min(3, int(float(influencer["impact_score"]) / 35) + 1))
        ranked_coins = sorted(coins, key=lambda item: float(item["hype_score"]), reverse=True)[:max_links]
        for coin in ranked_coins:
            weight = (float(influencer["impact_score"]) * 0.5) + (float(influencer["trust_score"]) * 0.3) + (float(coin["hype_score"]) * 0.2)
...
def train_all_models(db: Database) -> dict[str, dict]:
